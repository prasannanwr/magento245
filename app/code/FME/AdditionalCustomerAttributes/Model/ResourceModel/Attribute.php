...
class Attribute extends \Magento\Catalog\Model\ResourceModel\Attribute
{
    ...
    protected function _updateRelatedSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete(
            $this->getTable('fme_additionalcustomerattributes_customer_group'),
            $delCondition
        );
        $this->getConnection()->delete(
            $this->getTable('fme_additionalcustomerattributes_stores'),
            $delCondition
        );
        $store_ids = $object->getStoreIds();
        foreach ($store_ids as $store_id) {
            $entityRow = ['attribute_id' => $object->getId(),'store_id' => $store_id];
            $this->getConnection()->insert($this->getTable('fme_additionalcustomerattributes_stores'), $entityRow);
        }
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $bind = [':attribute_id' => $object->getId()];
        $select = $connection->select()->from(
            ['fme_stores' => $this->getTable('fme_additionalcustomerattributes_stores')],
            ['store_id']
        )->where(
            'attribute_id = :attribute_id'
        );
        ...
        $select = $connection->select()->from(
            ...
            ['fme_group' => $this->getTable('fme_additionalcustomerattributes_customer_group')],
            ['group_id']
        )->where(
            'attribute_id = :attribute_id'
        );
