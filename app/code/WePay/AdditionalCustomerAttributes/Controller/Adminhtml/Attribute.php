 ...
 */
namespace WePay\AdditionalCustomerAttributes\Controller\Adminhtml;
...
abstract class Attribute extends \Magento\Backend\App\Action
{
     ...
     * @see _isAllowed()
     */
    const ADMIN_RESOURCE = 'WePay_AdditionalCustomerAttributes::attributes_attributes';
    ...
    public function createActionPage($title = null)
    {
        ...
        $resultPage->addBreadcrumb(__('WePay'), __('WePay'))
            ->addBreadcrumb(__('Additional Customer Attributes Fields'), __('Additional Customer Attributes Fields'))
            ->setActiveMenu('WePay_Extensions::wepayextensions')
            ->setActiveMenu('WePay_AdditionalCustomerAttributes::attributes');
        if (!empty($title)) {
            $resultPage->addBreadcrumb($title, $title);
        }
        $resultPage->getConfig()->getTitle()->prepend(__('Additional Customer Attributes Fields'));
        return $resultPage;
    }
